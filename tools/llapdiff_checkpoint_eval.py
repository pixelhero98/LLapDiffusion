...
def _evaluate_impute_case(
    ...
    *,
    ...
    for xb, yb, meta in test_dl:
        (V, T), yb, mask_bn = tv._sanitize_batch(xb, yb, meta["entity_mask"], device)
        if not mask_bn.any():
            continue
    ...
    return {
        "hidden_mae": abs_sum / max(1.0, elts),
        "hidden_mse": sq_sum / max(1.0, elts),
        "hidden_crps": crps_sum / max(1.0, crps_elts),
        "observed_mae": obs_abs_sum / max(1.0, obs_elts),
        "observed_token_frac": observed_token_sum / max(1.0, candidate_token_sum),
        "hidden_token_frac": hidden_token_sum / max(1.0, candidate_token_sum),
    }
...
def evaluate_checkpoint(
    ...
    *,
    ...
    test_sampling = tv._sampling_kwargs(cfg, prefix="TEST", fallback_prefix="EVAL")
