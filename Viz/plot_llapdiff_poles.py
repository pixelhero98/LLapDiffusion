...
from configs.config_utils import clone_config
from configs.dataset_archives import configure_dataset_archive
from configs.dataset_defaults import apply_dataset_preset, dataset_keys
from configs.dataset_registry import resolve_run_experiment
from Model.llapdiff import LLapDiff
from Model.llapdiff_utils import EMA, set_torch
...
def _align_config_to_model(cfg, pred: int, model_kwargs: Dict[str, object]) -> None:
    ...
    if data_dim is not None:
        ...
        cfg.VAE_CKPT = str(
            Path(cfg.VAE_DIR) / f"pred-{int(pred)}_ch-{int(data_dim)}_elbo.pt"
        )
        cfg.SUM_CKPT = str(
...
def _collect_conditioning(
    ...
    *,
    ...
    for xb, yb, meta in loader:
        (V, T), _, mask_bn = tv._sanitize_batch(xb, yb, meta["entity_mask"], device)
        if not mask_bn.any():
            continue
