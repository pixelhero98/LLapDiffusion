...
def _match_dt_to_horizon(
    ...
    """Return ``dt_flat`` only when it matches the latent horizon length."""
    if dt_flat is None:
        return None
    if dt_flat.dim() != 2:
        return None
...
def _load_module_state(module: nn.Module, state_dict: Dict[str, torch.Tensor], *, strict: bool = False) -> None:
...
def evaluate_val_diagnostics(
    ...
    *,
    ...
    metric_value = raw_sum / max(1, num_samples)
...
def evaluate_irregular_time_checks(
    ...
    *,
    ...
    if batches == 0:
        return {
            "ctx_delta_no_dt": float("nan"),
            "ctx_delta_no_xmask": float("nan"),
            "ctx_delta_zero_tsig": float("nan"),
        }
...
def _sampling_kwargs(
    config_obj: object, *, prefix: str = "EVAL", fallback_prefix: Optional[str] = None
) -> Dict[str, object]:
    def _read(name: str, *, default, aliases: Tuple[str, ...] = ()):
        names = [f"{prefix}_{name}"]
        if fallback_prefix is not None:
            names.append(f"{fallback_prefix}_{name}")
...
def evaluate_regression(
    ...
    mae = abs_sum / max(1.0, elts)
    mse = sq_sum / max(1.0, elts)
    crps = crps_sum / max(1.0, crps_elts)
    pinball = {q: (pinball_sums[q] / max(1.0, elts)) for q in pinball_sums.keys()}
...
def run(
    ...
    if init_ckpt_raw not in {None, "", False}:
        ...
        print(f"[init] loaded diffusion weights from {init_ckpt_path}")
    ...
    def train_one_epoch(epoch: int) -> Dict[str, object]:
        ...
        for xb, yb, meta in train_dl:
            ...
            mu_norm = mu_norm * obs_any.unsqueeze(-1).to(dtype=mu_norm.dtype)
            if not _is_finite_tensor(mu_norm):
                print("[warn] non-finite latent targets detected; skipping step")
                continue
            ...
            if (not _is_finite_tensor(x_t)) or (not _is_finite_tensor(eps_true)):
                print("[warn] non-finite noisy latent sample detected; skipping step")
                continue
            ...
            if not _grads_are_finite_params(grad_params):
                print("[warn] non-finite gradients detected; skipping optimizer step")
                optimizer.zero_grad(set_to_none=True)
                scaler.update()
                continue
            ...
            if grad_clip > 0:
                ...
                if not torch.isfinite(torch.as_tensor(grad_norm)):
                    print("[warn] non-finite gradient norm detected; skipping optimizer step")
                    optimizer.zero_grad(set_to_none=True)
                    scaler.update()
                    continue
        ...
        epoch_raw_loss = running_raw_loss / max(1, num_samples)
        epoch_stats = {
            "loss": epoch_loss,
            "raw_loss": epoch_raw_loss,
            "cond_fraction": cond_weight_sum / max(1, num_samples),
    ...
    test_metrics = evaluate_regression(
        ...
        self_cond=bool(getattr(config, "SELF_COND", False)),
        **_sampling_kwargs(config, prefix="TEST", fallback_prefix="EVAL"),
    )
