...
Baseline adapters are packaged under `llapdiffusion.baselines`. DLinear, NeuralCDE, PatchTST, TimeGrad, mTAN, t-PatchGNN, ContiFormer, and CSDI use pinned external upstream repositories; clone those sources outside this repository and pass their parent directory explicitly. Note, `MR-Diff is implemented first-party in this repository from the ICLR 2024 paper, and it has no GitHub repo or official implementation`:
...
The primary extrapolation benchmark keeps baselines target-only, since most are univariate across multiple series. Each target-only adapter receives target observation masks, so the baseline can see the missing target history. Full non-target covariates and per-feature covariate masks are only provided to baselines that naturally support, i.e., `--input-policy all_features` is exposed as an additional knob, currently supports only DLinear and PatchTST; the other extrapolation adapters remain target-only as naturally expected, and their result rows report the effective `input_scope`, `missingness_scope`, and `input_policy_effective`.

LLapDiff is a joint multivariate model. It conditions on all context features, per-feature observation masks, and a globally shared query-time grid across entities. The baseline result files and `baseline_pool_notes.json` also report `comparison_type`, `modeling_scope`, and `time_feature_protocol`. Baseline normalized time/gap features use context `delta_t`; future `delta_t_y` is used only as known query-grid metadata, not as target values.

PhysioNet is marked as `patient_relative_contiguous_split` in the result metadata. It is kept as a special patient-relative setting because there are not enough unique horizon windows for the same purged-horizon protocol used by the regular public panels.
