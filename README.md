...
The primary extrapolation benchmark keeps baselines target-only. Each target-only adapter receives target observation masks, so missing target history is visible to the baseline. Full non-target covariates and per-feature covariate masks are not part of the primary baseline input. `--input-policy all_features` is exposed for ablations, but currently changes only DLinear and PatchTST; the other extrapolation adapters remain target-only and their result rows report the effective `input_scope`, `missingness_scope`, and `input_policy_effective`.

LLapDiff is reported as a joint global model. It conditions on all context features, per-feature observation masks, and a globally shared query-time grid across entities. The baseline result files and `baseline_pool_notes.json` also report `comparison_type`, `modeling_scope`, and `time_feature_protocol`. Baseline normalized time/gap features use context `delta_t`; future `delta_t_y` is used only as known query-grid metadata, not as target values.

PhysioNet is marked as `patient_relative_contiguous_split` in result metadata. It is kept as a special patient-relative setting because there are not enough unique horizon windows for the same purged-horizon protocol used by the regular public panels.
